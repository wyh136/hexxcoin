...
void CWallet::ReacceptWalletTransactions()
{
    ...
    while (fRepeat)
    {
        ...
        BOOST_FOREACH(PAIRTYPE(const uint256, CWalletTx)& item, mapWallet)
        {
            ...
            if ((wtx.IsCoinBase() ||  wtx.IsZerocoinSpend()) && wtx.IsSpent(0))
                continue;
...
bool CWallet::CreateZerocoinMintTransaction(const vector<pair<CScript, int64> >& vecSend,
                               CWalletTx& wtxNew, CReserveKey& reservekey, int64& nFeeRet, std::string& strFailReason, const CCoinControl* coinControl)
{
   int64 nValue = 0;
   BOOST_FOREACH (const PAIRTYPE(CScript, int64)& s, vecSend)
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            BOOST_FOREACH(const CZerocoinSpendEntry& item, listCoinSpendSerial){
                if(spend.getCoinSerialNumber() == item.coinSerial){
                    ...
                    pubCoinTx.nHeight = zerocoinSelected.nHeight;
...
bool CWallet::CreateZerocoinMintTransaction(CScript pubCoin, int64 nValue,
                                CWalletTx& wtxNew, CReserveKey& reservekey, int64& nFeeRet, std::string& strFailReason, const CCoinControl* coinControl)
{
    vector< pair<CScript, int64> > vecSend;
...
string CWallet::MintZerocoin(CScript pubCoin, int64 nValue, CWalletTx& wtxNew, bool fAskFee)
{
    // Check amount
    if (nValue <= 0)
        return _("Invalid amount");
